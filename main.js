...
/* Phase 2 — language preference (content translation remains a separate localization pass) */
document.addEventListener("DOMContentLoaded",()=>{const buttons=document.querySelectorAll("[data-cv-lang]"),status=document.getElementById("cvLanguageStatus");if(!buttons.length||!status)return;const names={en:"English",hi:"हिन्दी",gu:"ગુજરાતી"};const key="cv_language";const apply=lang=>{buttons.forEach(b=>b.classList.toggle("active",b.dataset.cvLang===lang));status.textContent=names[lang]+" selected.";localStorage.setItem(key,lang);document.documentElement.lang=lang==="en"?"en":lang==="hi"?"hi":"gu"};buttons.forEach(b=>b.addEventListener("click",()=>apply(b.dataset.cvLang)));apply(localStorage.getItem(key)||"en")});
